...
namespace BestStore.Data.Repositories
{
    ...
    {
        private readonly BestStoreDbContext _dbContext;
        public ShipAddressRepository(BestStoreDbContext context)
        {
            _dbContext = context;
        }
        ...
        {
            Province province = await _dbContext.Provinces.SingleOrDefaultAsync(p => p.ProvinceID == provinceID);
            if (null == province)
                return string.Empty;
            else
        ...
        {
            City city = await _dbContext.Cities.SingleOrDefaultAsync(c => c.CityID == cityID);
            if (null == city)
                return string.Empty;
            else
